...
/*
 * extern "C"： 必须采用C的编译方式，为什么，请看JNIEnv内部源码,无论是C还是C++ 最终是调用到 C的JNINativeInterface，所以必须采用C的方式 extern "C" 函数的实现
 * */
...
Java_com_derry_as_1jni_1project_MainActivity_getStringPwd (JNIEnv * env, jobject jobj) {
}
...
Java_com_derry_as_1jni_1project_MainActivity_changeName(JNIEnv *env, jobject thiz) {
   ...
   // 打印字符串  目标 printf 和 std::cout都不能使用
...
Java_com_derry_as_1jni_1project_MainActivity_changeAge(JNIEnv *env, jclass jcls) {
    const char * sig = "I";
    jfieldID j_fid = env->GetStaticFieldID(jcls, "age", sig);// 静态的
...
Java_com_derry_as_1jni_1project_MainActivity_callAddMethod(JNIEnv *env, jobject job) {
    ...
    jmethodID j_mid = env->GetMethodID(mainActivityClass, "add", "(II)I"); //(II)表示参数类型，后面的I表示返回值
